...
use super::test_client::{TestClientData, TestClient, TestResult, Step};
...
impl TestMqttClient {
    pub fn new(id: usize, host: &str, port: u16, scenario_map: Value, rx: Receiver<bool>) -> Self {
        ...
        let credentials = scenario_map.get("scenario").unwrap().get("credentials").unwrap();
            ...
            while client_data.rx().is_empty() {
                for step in client_data.steps() {
                    ...
                    if step.contains_key(&publish) {
                        let client = client.lock().await;
                        let _publish = client.publish("some/topic", rumqttc::QoS::ExactlyOnce, false, "hej".as_bytes().to_vec()).await;
                        // tokio::time::sleep(Duration::from_secs(1)).await;
                    }
                }
            }

            let _stop = client.lock().await.publish("stop", rumqttc::QoS::ExactlyOnce, false, "stop").await;
        });
